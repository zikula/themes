<h1>{$info.title}</h1>
<p class="small"><img src="{$imagepath}/clock.gif" alt="" /> {$info.time|date_format:"%B %d, %Y"}</p>
{$preformat.fulltext}
...
{nmodcallhooks hookobject=item hookaction=display hookid=$info.sid module=News returnurl=$returnurl}
