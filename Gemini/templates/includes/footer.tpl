<div id="footer">
    <p class="credit">
        &copy;&nbsp;2006-{'Y'|date}&nbsp;<strong><a href="{homepage}" title="{sitename} {gt text="Home Page"}">{sitename}</a></strong>&nbsp;|&nbsp;
        <a href="{modurl modname='Sitemap'}" title="{gt text="Easily find information across our site via our Sitemap"}">{gt text="Sitemap"}</a>&nbsp;|&nbsp;
        <a href="{modurl modname='formicula'}" title="{gt text="Contact us"}">{gt text="Contact"}</a>&nbsp;|&nbsp;
        <a href="{modurl modname='News' type='user' func='view' theme='rss'}" title="{gt text="Subscribe link for our News RSS feed"}">{gt text="RSS FEED"}</a>&nbsp;|&nbsp;
        {gt text="Valid"}&nbsp;<a href="http://jigsaw.w3.org/css-validator/check/referer" title="{gt text="Validate the CSS of this web page"}"><strong>{gt text="CSS"}</strong></a>&nbsp;|&nbsp;
        <a href="http://validator.w3.org/check/referer" title="{gt text="Validate the XHTML of this web page"}"><strong>{gt text="XHTML"}</strong></a><br />
        {* Please leave this if you use our template. Thank you *}
        <a href="http://geeksmakemehot.com/themes/" title="{gt text="Link to the website of"} geeksmakemehot.com/themes/">{gt text="Gemini"}</a>&nbsp;{gt text="brought to you by"}&nbsp;<a href="http://geeksmakemehot.com/">geeksmakemehot.com</a>&nbsp;|&nbsp;{gt text="Developed by"}&nbsp;<a href="http://atthe404.com" title="{gt text="Link to the website of"} atthe404.com">{gt text="Root"}</a>&nbsp;|&nbsp;{gt text="Design by"}&nbsp;<a href="http://geeksmakemehot.com/" title="{gt text="Link to the website of"} geeksmakemehot.com">geeksmakemehot.com</a>
        {* Please leave this if you use our template. Thank you *}
    </p>
</div>