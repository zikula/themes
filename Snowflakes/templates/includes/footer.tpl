{*footer starts here*}
<div id="footer">
    <p id="copy">
        &copy;&nbsp;2007-{'Y'|date}&nbsp;<a href="{homepage}" title="{sitename}&nbsp;{gt text="Home Page"}"><strong>{sitename}</strong></a>&nbsp;&#183;&nbsp;
        <a href="{modurl modname='Sitemap'}" title="{gt text="Easily find information across our site via our Sitemap"}">{gt text="Sitemap"}</a>&nbsp;&#183;&nbsp;
        <a href="{modurl modname='formicula'}" title="{gt text="Contact us"}">{gt text="Contact"}</a>&nbsp;&#183;&nbsp;<br />
        {gt text="design by"}:&nbsp;<a href="http://www.free-css-templates.com/" title="{gt text="Link to the website of Free CSS Templates"}">{gt text="Free CSS Templates"}</a>&nbsp;&#183;&nbsp;
        {gt text="Valid"}&nbsp;<a href="http://jigsaw.w3.org/css-validator/check/referer" title="{gt text="Validate the CSS of this web page"}"><strong>{gt text="CSS"}</strong></a>&nbsp;&#183;&nbsp;
        <a href="http://validator.w3.org/check/referer" title="{gt text="Validate the XHTML of this web page"}"><strong>{gt text="XHTML"}</strong></a>
    </p>
    <p id="feed">
        <a href="{modurl modname='News' type='user' func='view' theme='rss'}" id="rss" title="{gt text="Subscribe link for our News RSS feed"}">{gt text="RSS Feed"}</a>
    </p>
</div>
